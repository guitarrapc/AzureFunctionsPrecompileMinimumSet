...
FOR /F %%CSPROJ IN ('dir /b "%DEPLOYMENT_SOURCE%\src" /ad') DO (
  if /i "%%CSPROJ"==".vs" (
    echo skipping %%CSPROJ
  ) else if "%%CSPROJ"=="packages" (
    echo skipping %%CSPROJ
  ) else (
    call :ExecuteCmd "%MSBUILD_PATH%" "%DEPLOYMENT_SOURCE%\src\%%CSPROJ\%%CSPROJ.csproj" /nologo /verbosity:m /t:Build /p:Configuration=Release;OutputPath="%DEPLOYMENT_TEMP%\%%CSPROJ";UseSharedCompilation=false %SCM_BUILD_ARGS%
    IF !ERRORLEVEL! NEQ 0 goto error
  )
)
